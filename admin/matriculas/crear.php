...
require_once("../../clase/ClaseMatricula.php"); // llamo al modulo usuario
  $usuarioss=new Matricula(); // ccreo mi instancia de usuario
  $datoss=$usuarioss->get_estudiante();  // llamamos al metodo get_ususario de userModulo
  require_once("../../clase/ClaseCurso.php"); // llamo al modulo usuario
  $usuarioss=new Curso(); // ccreo mi instancia de usuario
  $dato_curso=$usuarioss->get_Curso();  // llamamos al metodo get_ususario de userModulo


if(isset($_POST["grabar"]) and $_POST["grabar"]=="si"){ // que si se enviaron los datos
  require_once("../../clase/ClaseMatricula.php"); // llamo al modulo usuario
  $usuario=new Matricula(); // ccreo mi instancia de usuario
  $usuario->agregar_matricular(); // llamo al metodo de agregar programa por medio de mi instancia
  exit(); // cierro
}
?>
<!DOCTYPE html>
...
<body id="page-top">
 ...
 <!-- inicio del menu lateral -->
  ...
  ?> 
                  <h3 class="box-title"><b>Matricular materia estudiante</b></h3>
                ...
                <?php } ?></select>
                   <span class="help-block">
                                        <strong style="color:red">{{ estudiante }}</strong>
                            </span>
                   </div>
                   ...
                   <div class="form-group">
                    ...
                    <option value="" disabled="" selected="">seleccione materia</option>
                              <?php for($i=0; $i<sizeof($dato_curso); $i++) {?>
                                    <option value="<?php echo $dato_curso[$i]["id"] ?>"> <?php echo $dato_curso[$i]["materia"]; ?> </option>
                <?php } ?></select>
                 <span class="help-block">
                                        <strong style="color:red">{{ materia }}</strong>
                            </span>
                   </div>
                   </div>
